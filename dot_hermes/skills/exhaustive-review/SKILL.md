...
description: "Use for an exhaustive review, audit, or simplification of an explicit git path, range, or commit."
---
...
For more than 15 files, process fresh-context batches of at most 8 in manifest
order. One parent owns the manifest; delegated batches only read and report.
...
## Maintenance — keep the installed copy in sync

An authoritative copy of this skill lives in the user's Stow-managed repo at
`~/.stow_repository/pi/.pi/agent/skills/exhaustive-review/`. The installed copy
under `~/.hermes/skills/exhaustive-review/` can silently drift behind it (this
once caused an older version to write `.review-manifest.csv` and `.review-lots`
into the repository under review). Before any review, `diff -r` both copies and
sync the installed one from the versioned one — the versioned copy wins. Never
write any review artifact (manifest, lots, reports) inside the repository under
review: everything lives under `$TMPDIR` and is auto-deleted. Incident detail:
[references/stow-repo-pollution-2026-09.md](references/stow-repo-pollution-2026-09.md).
