...
# Mailbox aliases (himalaya v2.x syntax). Required whenever the
# server's mailbox names don't match himalaya's canonical names
...
> emails to recipients. Always use `mailbox.alias.X` (dotted keys, directly under
> `[accounts.NAME]`).
...
```bash
himalaya envelope list --mailbox "Sent"
```
...
Most shared commands support JSON output with `--json`:
...
- Message IDs are relative to the current mailbox; re-list after mailbox changes.
