...
`trash`) to whatever the server actually calls them. Himalaya v2.x
uses the `mailbox.alias.X` syntax (dotted keys, directly under
`[accounts.NAME]`):
...
Do not use the old folder-alias sub-section form or the singular
`alias` form; use the dotted keys shown above directly under
`[accounts.NAME]`.
