...
import { AGENT_STATUS_WORDS } from './words.ts'

/**
 * agent-status - hide persistent "Thinking..." from the transcript and raise ONE calm
 * working label below the chat while the agent is busy. Calm by design: the label is drawn once per
...
export default function agentStatus(pi: ExtensionAPI): void {
	const bag = createShuffleBag(AGENT_STATUS_WORDS)
