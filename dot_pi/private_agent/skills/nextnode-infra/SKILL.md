...
| `@nextnode-solutions/infrastructure` | CLI (private) | Config-driven CI/CD CLI - never published on npm (`0.0.0-development`), consumed from the monorepo by Actions; exposes a `./worker-types` library export |
| `@nextnode-solutions/worker-types` | CLI | Generates `worker-configuration.d.ts` for Cloudflare Workers from `nextnode.toml` (`bin: worker-types`) |
| `@nextnode-solutions/monitoring` | app (private) | Internal Astro 6 dashboard at `monitoring.nextnode.fr`, Tailscale-only, Navy design language |
...
- [ ] Date/time code uses `Temporal` from `@js-temporal/polyfill` — see Rule 4
