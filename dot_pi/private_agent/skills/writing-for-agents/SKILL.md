...
description: Writing documents for agents. Use when creating or editing skills or extensions, modifying AGENTS.md, writing an agent-facing doc (ARCHITECTURE.md, PRODUCT.md), or authoring a tool description.
---

Reference for writing any document an agent consumes: a skill, an extension, an `AGENTS.md`, an agent-facing doc (`ARCHITECTURE.md`, `PRODUCT.md`), a tool description, or a doc reached by a pointer. The packaging differs; the writing does not: the same levers make each one predictable, since the agent takes the same _process_ every run rather than producing the same output. Where a document lives is placement, governed by `harness-tuning` and `skills/AGENTS.md`; this reference is about the writing itself.
...
- **Context load** is the cost of always-loaded material (an `AGENTS.md` line, a skill description) on the agent's window — the brevity rule in `AGENTS.md` governs it.
...
- Keep each meaning in a **single source of truth**: one authoritative place, so changing the behaviour is a one-place edit (the rule itself lives in `AGENTS.md`). (A leading word is the accidental inverse: it repeats a token on purpose, never the meaning.)
