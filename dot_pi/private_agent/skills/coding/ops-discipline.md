...
## Context Discipline (Grep Before Read)
...
## Routing Is Not an Excuse to Skip
...
- **Cross-boundary debt still counts.** A value duplicated across
  languages that must stay in sync (see *DRY: One Source of Truth* in
  `SKILL.md`) is exactly the debt a cleanup pass exists to retire: make
  one side the single source of truth and have the other read it. "Two
  files" is not "out of scope" — if the edit must go through a specific
  agent, complete it, don't drop it.
