...
> **Provider-version caveat:** this file mixes the v4-only `cloudflare_workers_script` resource with the v5-only `cloudflare_worker`/`cloudflare_worker_version`/`cloudflare_workers_deployment` resources. Do not copy both together for a pinned provider version — verify the resource name and schema in the official documentation for your pinned provider version (v4 and v5 Worker resources differ).
...
> Uses the v5 Worker resource family (`cloudflare_worker`, `cloudflare_worker_version`, `cloudflare_workers_deployment`) — not mixable with the v4 `cloudflare_workers_script` above; verify against your pinned provider version.
...
resource "cloudflare_workers_deployment" "api" {
  ...
  worker_name = cloudflare_worker.api.name
  versions {
    version_id = cloudflare_worker_version.api_v1.id
    percentage = 100
  }
}
```
