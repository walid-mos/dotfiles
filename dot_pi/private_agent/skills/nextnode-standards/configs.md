...
The preset export is typed (`OxfmtConfig` with `ignorePatterns` required - it always ships them, consumers spread before layering), so an accidental removal of `ignorePatterns` fails type-check.
