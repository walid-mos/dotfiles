...
**Custom JS plugin** `nextnode` - bundled inside the package, 24 rules mechanizing the NextNode coding skills (no separate install).
...
| `no-magic-numbers` | error | Extract every unexplained literal into a named `const` (`enforceConst`: no `let` "constants"; 0, 1, -1 allowed; enums banned - value sets go in an `as const` object, per `nextnode/no-enum`) |
| `no-param-reassign` | error | No mutating params, incl. their props |
| `no-else-return`, `no-lonely-if`, `no-negated-condition`, `no-nested-ternary` | error | Flat, positive conditionals - a nested ternary hides its branching; flatten with early returns |
...
**Type-aware** (with `--type-aware`): `no-floating-promises`, `no-misused-promises`, `prefer-nullish-coalescing`, `switch-exhaustiveness-check` (fires even with `default` - new union members must surface), `no-for-in-array`, `prefer-optional-chain`, `no-unnecessary-condition` (`checkTypePredicates: true` - re-narrowing schema-parsed output is flagged; narrow `unknown` only at the I/O boundary) - all error.

**Disabled on purpose**: `oxc/no-async-endpoint-handlers` is `off` - its syntax-only detection also flags Hono handlers; Express 4 projects can opt in locally when they need rejection forwarding.
...
| `no-generic-runtime-guard` | error | No locally declared generic runtime guards (`isString(x: unknown): boolean`, `isRecord`, ... found by name or by `unknown`/`any` param + type predicate) - validate unknown input with the application schema library at its I/O boundary. A rare canonical low-level helper needs a documented, targeted `// oxlint-disable-next-line nextnode/no-generic-runtime-guard -- <why>`. Domain predicates (`isNumber(token: Token)`) and literal-union predicates are clean |
