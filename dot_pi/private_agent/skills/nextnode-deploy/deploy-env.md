...
- **One `<SIBLING_NAME_UPPER_SNAKE>_URL=https://<resolved-hostname>` per routed sibling, including self** — symmetric cross-service URL injection so no service needs to know who depends on whom (`buildServiceUrlEnv` in `domain/hetzner/service-env.ts`). The `https://` prefix is in the value, so app code consumes it verbatim — no concatenation in callers (commit `72eb402`). Internal services (no `url`) contribute nothing.
...
cloudflare-pages's secret pool IS `[deploy].secrets` directly — one deployable unit, no per-service split. hetzner-vps ALSO accepts `[deploy].secrets` as a GLOBAL pool, folded into every service by `resolveSecrets` (`config/validation/deploy.ts`, renamed from `resolveSecretPool`) via `expandServiceSecrets` — entries and auto-generated tables: see [config.md](config.md) `[deploy]` field table.
...
`updatePagesEnvVars()` in `adapters/cloudflare/pages/env.ts` PATCHes the Pages project's `deployment_configs.production.env_vars` — computed values as `plain_text`, secrets as `secret_text`. Only the `production` deployment config is pushed (preview is not used for static sites). PATCH merges with existing env vars - keys not in the payload are left untouched.
