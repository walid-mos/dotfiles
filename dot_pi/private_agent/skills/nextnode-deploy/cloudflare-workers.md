...
- **Provision = Terraform.** A pure `config → main.tf.json` generation, applied by a shell-out adapter. State lives on **HCP Terraform** (org `nextnode`), execution mode **local** (HCP stores state only; the CI runner executes the binary). Terraform owns every stateful resource: D1, KV, Queues, R2, DNS records, Redirect Rulesets, and the zone firewall rulesets (rate limiting + public paths).
...
| Owner | Resources |
|-------|-----------|
| **Terraform** | D1 databases, KV namespaces, Queues, R2 buckets (+ R2 custom domains), DNS records, Redirect Rulesets, zone rulesets (rate limiting + public-path firewall). The DNS zone is read as a `data` source, never a managed resource. |
| **wrangler** | The Worker script, its bindings (D1/KV/R2/Queues), its Custom Domain route, its cron triggers, its `ratelimits` bindings and `limits` caps. |
...
| `rate_limit` | Optional zone rate-limiting table — see Zone firewall below. Requires `url`. |
| `public_paths` | Optional firewall path allow-list — see Zone firewall below. Requires `url`. Absent ≠ empty: absent = no rule (whole Worker public), `[]` = every path blocked. |
| `limits` | Optional per-invocation caps `{ cpu_ms, subrequests }` — see Zone firewall below. |
| `rate_limiters` | Optional array of in-Worker rate-limiter bindings — see Zone firewall below. |
...
### Zone firewall — the four barriers (PR #86)

Four opt-in blocks on a worker's `[deploy.services.<name>]` table. Validated by `config/validation/deploy-worker-firewall.ts` (`workerFirewallFields`), typed in `config/worker-firewall.ts`, generated by `domain/cloudflare/workers/terraform-{rate-limit,firewall,rulesets}.ts` + `wrangler-config.ts`.
...
paths = ["/api/contact"]                  # required, non-empty
methods = ["POST"]                        # optional; []/absent = every method
requests_per_period = 5                   # required positive integer
period = 60                               # one of 10, 60, 120, 300, 600, 3600 (default 60)
mitigation_timeout = 600                  # one of 0, 10, 60, 120, 300, 600, 3600, 86400 (default 600)

[deploy.services.web.limits]              # per-invocation caps (wrangler-side)
cpu_ms = 5000                             # default 1000
subrequests = 50                          # default 50

[[deploy.services.web.rate_limiters]]     # in-Worker RateLimit bindings
name = "forms"                            # kebab, unique per service -> binding RL_FORMS
limit = 5
period = 60                               # 10 or 60 only (binding-accepted periods)
```

The four barriers, what each generates and what it costs:

| Barrier | Generated | Environments | Plan requirement |
|---------|-----------|--------------|------------------|
| `rate_limit` | One `cloudflare_ruleset` per declaring service (`ratelimit-<label>`, `phase: http_ratelimit`): `block` with a 429 `application/json` body, `ratelimit` characteristics `['ip.src','cf.colo.id']`, host+path(+method) expression | **production only** | Zone **Pro+** — the generated rule carries Host/Method/custom response/declared periods, which the Free plan's rate limiting rule cannot hold; the resource create is refused at apply on Free |
| `public_paths` | One single `cloudflare_ruleset` for the whole project (`firewall_public_paths`, `phase: http_request_firewall_custom`), one `block` rule per routed declaring worker, expression `(<host> and not (<paths>))`; empty list → the whole host is blocked. **No `action_parameters`** (Free reserves custom responses) | **production only** | Works on a **Free** zone (≤ 5 custom rules) |
| `limits` | Wrangler `limits: { cpu_ms, subrequests }` on EVERY worker (defaults 1000 / 50, overridden field by field) | **both** | **Workers Paid** (account-scoped, independent of the zone plan) — `wrangler deploy` refuses `cpu_ms` on a Free account (`code: 100328`); `subrequests` needs wrangler ≥ 4.62 |
| `rate_limiters` | Wrangler `ratelimits` array: binding `RL_<NAME>` (`toBindingName`: uppercase, `-`→`_`), `namespace_id = FNV-1a 32-bit` of `[project, environment, worker, limiter].join('/')` (the `/` separator is kebab-proof: `api` + `forms-v2` cannot collide with `api-forms` + `v2`), `simple: { limit, period }` | **both** (the namespace id is environment-scoped) | Unaffected by zone plan |
...
**Dev is deliberately ungated**: both zone rulesets are production-only, so development answers on `dev.<host>` with no rate limit and no public-path gate. `rate_limiters` bindings and `limits` caps, wrangler-side, run in both environments.
...
- Resources: `cloudflare_d1_database` (when D1 declared), `cloudflare_workers_kv_namespace`, `cloudflare_queue`, `cloudflare_r2_bucket` (+ `cloudflare_r2_custom_domain` for `cdn` buckets), `planetscale_postgres_branch_role` + `cloudflare_hyperdrive_config` (when `[services.planetscale]` declared — provider `planetscale/planetscale ~> 1.5` added only then), plus DNS records, Redirect Rulesets, and the zone firewall rulesets. The zone rules come from one pure derivation: `deriveZoneRules` (`zone-rules.ts`) → `deriveRoutedWorkers` filters workers that declare a barrier AND carry a `url`, returns `[]` for any environment other than `production` (a zone owns one entry point per phase — dev/prod workspaces would overwrite each other); the three families (redirects, rate limits, firewall) are merged into one `resource.cloudflare_ruleset` map by `mergeRulesetFamilies` — merging, never overwriting, or one family would delete the others.
...
- One `cloudflare_ruleset` (`kind: zone`, `phase: http_request_dynamic_redirect`) with a `redirect` action, **301** status, `expression: (http.host eq "<domain>" or http.host eq "www.<domain>")`, target `concat("<siteUrl>", http.request.uri.path)`, `preserve_query_string: true`.
...
- `main: <entry>`.
- `compatibility_date: "2026-06-01"` (`DEFAULT_WORKERS_COMPATIBILITY_DATE` — a fixed date freezing the workerd behaviour set so a deploy is reproducible run to run; a bump is a deliberate, reviewed change).
...
- `limits: { cpu_ms, subrequests }` — **always emitted** (defaults 1000 / 50, `DEFAULT_WORKER_CPU_MS` / `DEFAULT_WORKER_SUBREQUESTS`; per-service `[deploy.services.<name>.limits]` overrides field by field). Requires Workers Paid (`code: 100328` on Free).
- `ratelimits` — one `RL_<NAME>` binding per `[[deploy.services.<name>.rate_limiters]]` entry, with an FNV-1a `namespace_id` scoped to `project/environment/worker/limiter` and `simple: { limit, period }`. Omitted entirely when none is declared. See Zone firewall below.
...
The `--remote` flag targets the provisioned database (not a local one). The owning service is the first declaring `needs = ["d1"]`. The migrate job runs on **every deploy** when `[services.d1]` is declared (gated only by plan's `has_d1` output) — `wrangler d1 migrations apply --remote` is idempotent (it lists the remote `d1_migrations` table and applies only what is pending), and the deployed D1 (which a provision can recreate empty) is the source of truth, not the push's file diff. There is NO `detect-migration-changes` gating on this target (that git-diff gating remains for postgres on `deploy.yml` only).
...
- **Single source of truth** (`domain/cloudflare/workers/worker-env-types.ts`): types are rendered from the **SAME `WranglerDocument`** `buildWranglerConfig` deploys, fed placeholder provision outputs (types need only binding NAMES, not ids). So a binding can never be typed-but-not-deployed or vice versa. Mapping: `assets` + sibling service bindings → `Fetcher`; `d1_databases` → `D1Database`; `kv_namespaces` → `KVNamespace`; `r2_buckets` → `R2Bucket`; `queues.producers` → `Queue`; `ratelimits` → `RateLimit` (while `limits` is deliberately **never typed** — a deploy ceiling, not a binding); Hyperdrive → its CF type; public `vars` + `secrets` → `string`. `@cloudflare/workers-types` supplies the globals (`/// <reference types="@cloudflare/workers-types" />`).
- **Committed**: unlike the deploy-only wrangler config, this file is **committed in the consumer repo** (types are needed at typecheck locally and in every CI job, with no infra dependency at build). Regenerate it locally on config change — via the infra CLI (`generate-worker-types`, which requires `PIPELINE_CONFIG_FILE`) or the standalone published package `@nextnode-solutions/worker-types` (`bin: worker-types`, e.g. `worker-types gen --config apps/web/nextnode.toml`), which reuses the infra `generateWorkerTypesFromFile` export.
...
```
plan --+-- check-secrets (needs plan)
       |
       +-- quality (matrix; prod-gate folded in when producing prod)
       |
       +-- provision (needs plan + quality + check-secrets)
       |
       +-- migrate (needs plan + provision; if has_d1 — runs every deploy, wrangler apply is idempotent)
       |
...
- **`check-secrets` is the second job** — it fails the pipeline in seconds when a `[deploy].secrets` name is missing from GitHub, before quality/provision burn minutes.
...
| `PIPELINE_ENVIRONMENT` | all | `development` / `production` |
...
| `TF_TOKEN_app_terraform_io` | provision, migrate, deploy, teardown, plan-infra | HCP Terraform token (sourced from the `TF_CLOUD_TOKEN` org secret) — read natively by Terraform |
...
- **Org secret `TF_CLOUD_TOKEN`.** The HCP state backend token is a cross-project credential → an **org** secret (SKILL.md rule 10), consumed by the workflows as `TF_TOKEN_app_terraform_io`. See [github-org.md](github-org.md) — never hardcode the value.
