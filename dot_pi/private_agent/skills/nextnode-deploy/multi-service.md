...
- Each `build` service gets `healthcheck: wget -q -O- http://127.0.0.1:<port>/healthz` (interval 10s, timeout 3s, retries 6) — `127.0.0.1`, not `localhost` (which can resolve to `::1` and fail against an IPv4-only bind)
