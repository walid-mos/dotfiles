...
interface WorkerServiceConfig {
  ...
  readonly rateLimit?: WorkerRateLimitConfig   // { paths, methods?, requestsPerPeriod, period? = 60, mitigationTimeout? = 600 }
  readonly publicPaths?: ReadonlyArray<string> // absent = no rule; [] = every path blocked
  readonly limits?: WorkerLimitsConfig         // { cpuMs?, subrequests? } — overrides of the wrangler defaults 1000/50
  readonly rateLimiters?: ReadonlyArray<WorkerRateLimiterConfig> // { name (kebab, unique per service), limit, period (10|60) }
}
...
On the `cloudflare-workers` target each `[deploy.services.<name>]` block is **one Worker**, not a container (`WorkerServiceConfig`). `url`, `secrets`, `needs`, `depends_on` keep their meaning; the new fields are `entry` plus the four firewall blocks, and every container knob is rejected.
...
| `rate_limit` | table | No | — (absent = no rule) | Zone rate-limiting rule: `paths` (non-empty), `methods` (`[]`/absent = all), `requests_per_period`, `period` ∈ {10,60,120,300,600,3600} (60), `mitigation_timeout` ∈ {0,10,60,120,300,600,3600,86400} (600). Requires `url`. Zone **Pro+**. |
| `public_paths` | `string[]` | No | — (absent ≠ empty) | Paths that stay open; the rule blocks the negation. `[]` = every path blocked. Requires `url`. Works on Free (≤ 5 per zone, project-wide). |
| `limits` | table | No | — | Per-invocation caps: `cpu_ms` (default 1000), `subrequests` (default 50). Emitted for EVERY worker; requires **Workers Paid** (`code: 100328`). |
| `rate_limiters` | table-array | No | — | In-Worker `RateLimit` bindings: `name` (kebab, unique per service → `RL_<NAME>`), `limit`, `period` ∈ {10, 60}. FNV-1a namespace id keyed on `project/env/worker/limiter`. |

Path entries in `rate_limit.paths` / `public_paths` follow one grammar: start with `/`, exact path or single **trailing** `*` (prefix) — no mid-string wildcard, no quote/backslash. Full semantics, generated Terraform, and plan gates: [cloudflare-workers.md](cloudflare-workers.md).
