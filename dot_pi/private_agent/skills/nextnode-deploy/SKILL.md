...
See [cron-service.md](cron-service.md) — `[[deploy.cron]]` declares scheduled HTTP jobs entirely in `nextnode.toml` (Hetzner only), rendered into a single BusyBox `cron` sidecar; on Workers they map to native cron triggers instead. See [cloudflare-workers.md](cloudflare-workers.md).
