...
**Source**: @nextnode/core @ 649362a (verified 2026-09-24 against packages/infrastructure, incl. PR #86 zone firewall)
...
| `check-secrets` | deploy | Fail-fast at the pipeline's first stage: verify every secret declared in `[deploy].secrets` exists in `ALL_SECRETS`, reporting ALL missing names at once (the same check `pickSecrets` runs inside `deploy`, hoisted so the run dies in seconds). Dedicated `check-secrets` job in all 3 deploy workflows; `provision`/`build-image` depend on it. See [pipeline.md](pipeline.md) |
...
| `migrate-remote` | deploy | Run migrations between `provision` and `deploy`, dispatched per database: postgres → `migrate_command` in an ephemeral container (no pre-migrate snapshot — wal-g covers it); D1 → `wrangler d1 migrations apply --remote`. No-op when neither `[services.postgres]` nor `[services.d1]` is declared. Postgres runs are gated by `detect-migration-changes` (`migrations_changed`); D1 runs every deploy (idempotent). See [postgres-service.md](postgres-service.md) and [cloudflare-workers.md](cloudflare-workers.md) |
| `seo-guard` | deploy | Inject `_headers` + `robots.txt` into build output for non-prod envs |
| `generate-worker-types` | deploy | Cloudflare Workers only (no-op elsewhere). Render `worker-configuration.d.ts` per Worker from the same `WranglerDocument` deploy uses, so `import { env } from 'cloudflare:workers'` is typed. Runs at typecheck/build time (locally or in CI), never at deploy; the file is committed in the consumer repo. Also available standalone via the published `@nextnode-solutions/worker-types` CLI (`bin: worker-types`), which reuses the infra `generateWorkerTypesFromFile` export. See [cloudflare-workers.md](cloudflare-workers.md) |
...
| `compute-image-ref` | deploy | Resolve the per-service `image_refs` JSON Record, render `docker-bake.json` from `nextnode.toml` (two-layer cache per target: ephemeral GHA scope + durable GHCR `:buildcache` registry scope), emit `bake_file`. See [pipeline.md](pipeline.md) |
...
See [observability-service.md](observability-service.md) — self-hosted VictoriaLogs + VictoriaMetrics + vmagent + vmalert + Alertmanager + blackbox as compose sidecars on the monitoring VPS, compose-only (no external provisioning), loopback ports + Caddy tailnet vhosts as the trust boundary.

## Scheduled jobs (cron)

See [cron-service.md](cron-service.md) — `[[deploy.cron]]` declares scheduled jobs entirely in `nextnode.toml`: on Hetzner a BusyBox `cron` sidecar fires an HTTP request; on Workers they map to native cron triggers (only `schedule`/`service` are used; `path`/`method` stay required at parse but are unused). Rejected on cloudflare-pages. See [cloudflare-workers.md](cloudflare-workers.md).

## SEO guard

Prevents search engine indexing of non-production deploys. `computeSeoGuardFiles(environment)` returns `_headers` (X-Robots-Tag: noindex) + `robots.txt` (Disallow: /) for non-prod, empty for prod; the `seo-guard` CLI writes them into the build output (Pages) and the Workers deploy injects them per asset-shipping service.
...
4. **Single source of truth for config defaults** - config defaults live as named constants (`config/types.ts`, `config/service-config.ts`); validators import these constants - they never define their own inline defaults. Wrangler-side constants live with the wrangler domain (`domain/cloudflare/workers/wrangler-document.ts`). If a concept doesn't apply to a project type, there must be NO default for it
...
10. **Project secrets are GitHub env-secrets, never org-secrets** — per-project credential → env-secret (`EnvSecretsAdapter`); cross-project credential (e.g. `TF_CLOUD_TOKEN`) → org-secret. See [github-org.md](github-org.md).
...
25. **`[deploy].secrets` is the GLOBAL secret pool — injected into every service (both targets)** — folded into each service via `expandServiceSecrets`. See [deploy-env.md](deploy-env.md) + [config.md](config.md).
...
31. **Workers reach each other on service bindings ONLY — never peer URLs** — a sibling in `needs` emits a `WranglerServiceBinding`; at runtime `env.<NAME>` is a `Fetcher` (RPC, no DNS/TLS/public hostname), and `buildWorkerVars` never injects a sibling `<NAME>_URL` (contrast the VPS). The binding graph also orders the sequential deploy (a cycle throws). See [cloudflare-workers.md](cloudflare-workers.md).
...
33. **`worker-configuration.d.ts` is generated from the deploy wrangler doc and committed** — `generate-worker-types` renders the SAME `WranglerDocument` deploy uses (placeholder outputs, names only) into the app package root, so a binding can never be typed-but-not-deployed or vice versa. Runs at typecheck/build time, never at deploy. The published `@nextnode-solutions/worker-types` CLI exposes the same generator standalone. See [cloudflare-workers.md](cloudflare-workers.md).
...
35. **Zone barriers are opt-in, plan-gated, and production-only** — the four `[deploy.services.<name>]` firewall blocks (`rate_limit`, `public_paths`, `limits`, `[[rate_limiters]]`) generate Cloudflare zone rulesets / wrangler caps / `RateLimit` bindings; the two zone-rule barriers (`rate_limit`, `public_paths`) require `url` (a zone rule matches on the host), while `limits`/`rate_limiters` are wrangler-side and do not; the zone rulesets are emitted in production only (dev is deliberately ungated), and the Free-plan ceilings are refused at load (1 rate-limiting rule/zone, 5 custom rules/zone). See [cloudflare-workers.md](cloudflare-workers.md).
