...
function entry(
	overrides: Partial<SurfaceEntry> & Pick<SurfaceEntry, 'id'>,
): SurfaceEntry {
	return {
...
void test('renders targets in priority order and clips each line to the width', () => {
	const registry = createSurfaceRegistry()
	registry.register(
		entry({ id: 'later', priority: 2, render: () => ['second\nignored'] }),
	)
	registry.register(
		entry({
			id: 'first',
...
void test('keeps every rendered line in the single normal view', () => {
	const registry = createSurfaceRegistry()
	registry.register(
		entry({
			id: 'details',
	...
	assert.deepEqual(
		[...registry.render('aboveEditor', 80)],
		['summary', 'detail', 'full detail'],
	)
})

void test('caps entries at maxLines with a truncation marker', () => {
	const registry = createSurfaceRegistry()
	...
	registry.register(
		entry({ id: 'capped-custom', maxLines: 2, render: () => manyLines }),
	)
...
void test('isolates render failures per entry without dropping siblings', () => {
	const registry = createSurfaceRegistry()
	registry.register(
		entry({
			id: 'broken',
	...
	registry.register(
		entry({ id: 'healthy', priority: 2, render: () => ['ok'] }),
	)
...
void test('truncates ANSI-colored lines without exposing escape fragments', () => {
	const registry = createSurfaceRegistry()
	registry.register(
		entry({ id: 'colored', render: () => ['\u001b[31mabcdef\u001b[0m'] }),
	)

	const line = registry.render('aboveEditor', 4)[0] ?? ''
	assert.equal(line.includes('\u001b[31m'), true)
	assert.equal(line.includes('\u001b[0m'), true)
	assert.equal(line.includes('\u001b[3'), true)
})

void test('replaces duplicate ids and makes stale/repeated unsubscriptions harmless', () => {
	const registry = createSurfaceRegistry()
	...
	const removeReplacement = registry.register(
		entry({ id: 'status', render: () => ['replacement'] }),
	)
...
void test('notifies subscribers on registration changes only while entries change', () => {
	const registry = createSurfaceRegistry()
	let changes = 0
